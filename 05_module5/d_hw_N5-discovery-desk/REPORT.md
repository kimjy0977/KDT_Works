...
| ① 의도 분류 | v2 0.925 → **v3 평균 0.955** (각 3회) · 폭 0.027 · 범위 밖 78.6% |
| ② 답변 통과율 | 골든셋 **100.0%** (2회 연속) · ★홀드아웃 **64.3%** |
| ★홀드아웃 | 1차 60.0% → 2차 78.6% → **3차 64.3%** · ★골든셋과의 격차가 14.7%p → **31.7%p** 로 «벌어졌다» |
| 규모 | 지식원 기사 72건 + 사실 카드 45장 · 라우팅 평가셋 55건 · 답변 정답셋 25건 |
| ★전제 | **이 100%를 «혼자» 읽으면 안 된다.** 그 25문항을 «보면서» 여섯 번 고쳤다. 한 번도 안 본 14문항에서는 **64.3%**다. 그리고 ★**직접 써 보니 두 번째 질문에서 막혔다** — 평가셋에 없는 질문이었다 |
...
| 지식원 | 무엇 | 규모 | 확실성 |
|---|---|---|---|
| `desk/facts_base.json` | 기준 사실 카드 — 내가 쓴 것 | 45장 | [정설]·[추정]·[논쟁] |
...
CSV의 `split` 열이 셋입니다 — `eval`(41) · `outscope`(14) · **`example`(11)**. 채점 코드(`load_seed`)는 `example`을 **읽지 않습니다.**
...
#### ★혼동행렬대로 고쳐 봤습니다 — 그리고 «1회로 판단한 내가 또 틀렸습니다»

「어떻게·언제가 붙어도 **대상이 있으면 그 라우트**」를 지침에 네 줄 더했습니다
(예시는 평가셋에 없는 말 — 삼엽충·성운 — 로 골라 `example` 로 등록했습니다).

**1회 재고 이렇게 적었습니다:**

```
v2 PALEO f1 0.875 · 누수 2건   →   v3 PALEO f1 1.000 · ★누수 0건
「누수가 2건 → 0건이 된 것은 숫자가 아니라 구조라 분명합니다」
```

**3회 재 보니 아니었습니다.**

```
v3  3회    0.946 · 0.946 · 0.973     평균 0.955 · 폭 0.027
    1회차  ★「화석의 색은 어떻게…」·「고생대와 중생대 경계는 언제…」 → CONCEPT
           ⇒ 누수 2건이 «그대로» 나왔다
    2·3회차 누수 0건
```

⇒ **누수는 사라지지 않았습니다. 회차마다 0~2건으로 흔들릴 뿐입니다.**
그리고 평균 0.925 → 0.955(+0.030)는 **폭(0.027)과 거의 같은 크기**라
**「개선」이라 부를 수 없습니다.**

★**오늘 같은 실수를 두 번 했습니다.** ②에서 「+4%p 개선」이라 했다가 폭이 16%p인 걸
알고 물렸고, ①에서 또 **1회 결과를 보고 「구조라 분명하다」**고 썼습니다.
**「구조」라는 말이 «한 번 본 것»을 확정으로 바꿔 주지 않습니다.**

★얻은 것이 없지는 않습니다 — **범위 밖 탐지가 78.6%로 «고정»**됐습니다
(v2는 78.6~85.7%로 흔들렸습니다). 평균은 그대로인데 흔들림이 줄었습니다.
다만 이것도 3회 표본이라 강하게 말하지 않겠습니다.

★그래서 **지침 4줄로는 PALEO → CONCEPT 누수를 못 고쳤습니다.**
모델이 policy의 기준보다 의문사에 더 반응하는 것을 **말로 막기 어렵다**는
쪽에 가깝습니다. 규칙 라우터는 이 문제가 없습니다 —
`hit["CONCEPT"] = 0`으로 **코드에서 껐기** 때문입니다.
다음에 할 일은 **LLM 라우터에도 같은 후처리를 붙여 보는 것**입니다.
...
| 8 | 라우팅 지침 v3 — 「의문사가 붙어도 대상이 있으면」 | 0.925 → 0.955 | — | ⚠ **폭(0.027)과 같은 크기** — 개선이라 못 한다 |
| 9 | 도구 지침 v4 — 「없다」 말하기 전 **둘 다** 본다 | — | 93.3% → 93.3% | 그 자체로는 안 움직였다 |
| — | *(교정) G17 `tools` 를 v4 규칙에 맞춤* | — | *93.3 → **96.0%*** | *개선 아님 — 지침을 고치고 정답셋을 안 고쳤다* |

★**「개선」과 「교정」을 갈라 적었습니다.** 6·7만 «내가 더 잘하게 만든 것»이고,
기울임들은 **«잘못 재던 것을 바로잡은 것»**입니다.

★★**그리고 이 표 전체에 붙는 단서가 하나 있습니다.**
이 점수들은 전부 **골든셋 25건**에서 나왔고, 저는 그 25건을 **보면서** 고쳤습니다.
같은 기간에 홀드아웃은 **60.0% → 78.6% → 64.3%**였고,
골든셋과의 격차는 **14.7%p에서 31.7%p로 벌어졌습니다.**
⇒ **표의 오른쪽으로 갈수록 숫자는 커지고 «믿을 만함»은 줄어듭니다.**
...
2차가 제 수정의 부작용을 잡았습니다. 그걸 고치고, 도구 규칙(v4)도 고치고,
정답셋 결함(G17)도 고쳤습니다. **골든셋은 1회차에 100%가 됐고 「항상 실패」가 0건이 됐습니다.**

그래서 **사실 카드를 10장 더 쓰고**(35 → 45장) **3차 홀드아웃 14건**을 만들어 쟀습니다.
...
**골든셋은 올라갔는데 홀드아웃은 내려갔고, 격차가 두 배 넘게 벌어졌습니다.**

⇒ 이 프로젝트에서 가장 분명하게 배운 것이 이것입니다:

> **같은 평가셋을 보며 고치면, 그 평가셋 점수는 오르고
> «처음 보는 것»과의 거리는 «벌어집니다».**
> 골든셋 96.0%는 이제 **성능이 아니라 «그 25문항에 대한 적합도»**입니다.

#### ★실패 5건 중 **3건이 제 잘못**이었습니다

| ID | 무엇이 일어났나 | 누구 문제인가 |
|---|---|---|
| **K11** | 「어제 발사된 로켓 탑재 중량」을 ESCALATE 로 정했는데 **기사에 실제로 있었다**(프로그레스 96 · 3톤) | ★**내 정답셋** — 2차 H21과 **똑같은 실수** |
| **K12** | 「연대를 **어떻게 잡나요**」라고 «방법»을 물어 놓고 ESCALATE 를 기대했다 | ★**내 문항이 모호** — 모델이 맞다 |
| K13 | 「화석 사진 감정」이 PALEO 로 분류돼 조회 → ESCALATE (REFUSE 여야) | 라우팅 |
| K01 | 「달은 어떻게 생겼나요」로 카드를 못 찾음 | 모델 |
| K10 | `get_term` 대신 `get_fact` — v3 에서 고친 자리가 재발 | 모델 |

★**2차에서 H21로 같은 실수를 겪고도 3차에서 K11을 또 했습니다.**
겪은 것을 **규칙으로 만들지 않았기 때문**입니다.

#### ★그래서 검사를 만들었더니 — 7건 중 **6건**이 잘못이었습니다

「ESCALATE 는 **못 찾아야** 정답인데, 정말 없는지 확인한 적이 없다」
...
**제가 만든 ESCALATE 문항 대부분이 「없을 것 같아서」 정한 것**이었습니다.
[10] 도달성 검사는 ANSWER 만 봤습니다 — **찾아야 정답인 쪽만** 보고,
**못 찾아야 정답인 쪽**은 아무도 안 봤습니다.

#### ⚠ 그런데 그 검사가 «과했습니다»

```
검색 「위성 궤도」  → 19건 «찾음»     그런데 실제 측정에서 G16 은 «통과»
검색 「공룡 발자국」 →  6건 «찾음»     G18 도 통과
```

`search_article` 은 토큰 매칭이라 느슨합니다. 19건이 나와도
**「한국 위성의 궤도 고도」는 그 안에 없습니다.** 모델이 옳게 넘긴 것입니다.

> ★**「찾았다」와 「답할 만하다」는 다릅니다 — 기계가 못 가리는 자리입니다.**

⇒ 검사를 **경고**로 낮추고 「**사람이 확인할 것**」을 붙였습니다.
**검사의 역할은 «자리를 가리키는 것»이지 «판정하는 것»이 아닙니다.**
지우면 K11 같은 진짜 결함을 놓치고, 실패로 두면 멀쩡한 문항을 틀렸다고 합니다.
...
```
Perplexity  「공룡은 약 6,600만 년 전, 백악기 말에 멸종했습니다.
             정확한 시점은 약 6,603만 8천 년 전으로 추정됩니다.」  [yna.co]

우리        「★비조류 공룡은 약 6,600만 년 전 … 멸종했습니다.
             이는 이리듐 이상층과 칙술루브 충돌구 연대를 근거로 한 정설입니다.」  [정설]
```

두 가지가 갈렸습니다.

1. **Perplexity는 「비조류」를 안 붙였습니다.** 조류는 살아남았는데요.
   우리 카드의 `caution`이 「«비조류» 공룡이다」라서 우리는 붙였습니다.
2. **Perplexity는 문장 뒤에 «출처»를 답니다. 우리는 같은 자리에 «확실성»을 답니다.**
   「어디서 왔나」와 「얼마나 믿을 수 있나」는 다른 질문입니다.

⇒ **같은 자리(문장 끝)를 쓰되 넣는 것이 다릅니다.** 이게 우리 화면의 정체성입니다.

### 베낀 것 · 살린 것

| 베낀 것 (검증된 대화 UI 문법) | 왜 |
|---|---|
| 단계 + **시간** 표시 (「조사 완료 9.1초」) | 우리는 20~60초라 **더** 필요하다. 빈 화면은 고장처럼 보인다 |
| 질문은 오른쪽·답변은 왼쪽 | 내 말과 답을 한눈에 가른다 |
| 답변 아래 **접이식 근거** | 필요할 때만 편다 |
| 「이어서 되물어도 됩니다」 | ★멀티턴이 노션 요건이다 |

| ★살린 것 (우리만의 것) | 무엇 |
|---|---|
| **확실성을 말풍선에 입힘** | 왼쪽 세로 컬러 바 + 등급 칩 — **스크롤만 해도** 단단한 답과 무른 답이 갈린다 |
| **넘기기 3갈래를 시각적으로 구분** | REFUSE(회색)·ESCALATE(보라)·ASK(청록) — **고치는 방법이 서로 다르다** |
| **근거를 파이프라인 순서로** | ①분류 → ②조회 → ③근거 → ④검증. 2차의 2열 그리드는 «흐름»이 안 보였다 |
| ★**지식원 패널** | 카드 45장을 **등급별로**([정설]25 [추정]10 [논쟁]9) — Perplexity의 「출처 N」 자리에 «무엇을 아는가»를 둔다 |
| ★**「지금 들어온 발표」** | 최근 72시간 기사에서 시작 질문을 만든다 |

★**「지금 들어온 발표」를 넣은 이유**: 고정 예시 넷은 **어제도 오늘도 같은 화면**이라 죽은 데모가 됩니다. 매일 수집하는 기사에서 뽑으면 **화면이 스스로 바뀌고**, 이 프로젝트의 핵심인 **[신설] 경로**가 시작 화면에서부터 드러납니다. 기사 제목은 **영어 그대로** 보여 줍니다 — 지식원이 영어라는 사실을 숨기지 않는 게 정직하고, 「한영 사전을 왜 만들었나」와도 이어집니다.
...
⚠ 평가셋이 여전히 작다        라우팅 55 · 답변 25 · 홀드아웃 10+14+14건
⚠ 홀드아웃을 «다 써 버렸다»    세 개 모두 이미 쟀다. 또 고치려면 카드를 더 써야 한다
...
★★ 고칠수록 «멀어졌다»        골든셋 74.7→96.0% 인데 홀드아웃 60.0→64.3%.
                             격차가 14.7 → 31.7%p 로 «벌어졌다».
                             ⇒ 같은 셋을 보며 고치면 그 셋에만 맞아 간다
★★ ESCALATE 문항 7건 중 6건이 결함  「없을 것 같아서」 정했고 지식원을
                             실제로 찾아본 적이 «한 번도» 없었다.
                             ⇒ 2차 H21 로 겪고도 3차 K11 에서 또 했다
...
```
1  ★★골든셋을 더 고치지 않는다 — 고칠수록 홀드아웃과 멀어진다.
   고칠 것이 보이면 «홀드아웃에서 재현되는지»부터 본다
2  ★ESCALATE 문항을 «전부 다시» 만든다 — 7건 중 6건이 지식원에 있는 것을
   「없다」고 정한 것이었다. 지식원을 «먼저 찾아보고» 정답을 정한다
3  ★앞으로 ② 는 «3회 평균»으로만 말한다 — 한 번 재서 나온 숫자는
   폭 안의 한 점일 뿐이다. 개선 판정 기준도 「폭을 넘었나」로 둔다
...
4  ★ESCALATE 문항을 «지식원을 먼저 찾아보고» 다시 만든다 —
   7건 중 6건이 「없을 것 같아서」 정한 것이었다
```
