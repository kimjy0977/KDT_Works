...
#### ★그리고 혼동행렬대로 고쳤더니 누수가 사라졌습니다

「어떻게·언제가 붙어도 **대상이 있으면 그 라우트**」를 지침에 **네 줄** 더했습니다
(예시는 평가셋에 없는 말 — 삼엽충·성운 — 로 골랐고 `example` 로 등록했습니다).

```
지침 v2 (평균 3회)   macro F1 0.925   PALEO f1 0.875   PALEO→CONCEPT 누수 2건
지침 v3 (1회)        macro F1 0.975   PALEO f1 1.000   ★누수 0건
```

혼동행렬이 **「어느 쪽으로 새는가」**를 보여 줬기 때문에 고칠 자리가 하나로 좁혀졌습니다.
f1만 봤다면 「PALEO가 좀 약하다」로 끝났을 것입니다.

⚠ **1회 측정입니다.** v2의 폭이 0.031이었으니 0.05 차이는 폭보다 크지만,
3회를 재지 않았으므로 **「확실히 개선」이라고까지는 말하지 않겠습니다.**
다만 **혼동행렬에서 누수가 2건 → 0건이 된 것**은 숫자가 아니라 구조라 분명합니다.
(대신 CONCEPT → SPACE 가 1건 새로 생겼습니다 — 「적색편이는 왜 생기나요」)
