...
| 지표 | 값 | 비고 |
|---|---|---|
| ① 의도 분류 — **gpt-5.6-luna** | v2 평균 0.925 → **v3 0.955** (각 3회) · 폭 0.027 | ★+0.030 은 폭과 «같은 크기» — 개선이라 못 한다 |
...
| ② 답변 통과율 — **gpt-5.6-terra** | 골든셋 **평균 96.0%** (3회) · 항상 실패 0건 | ★아래 홀드아웃과 «함께» 읽어야 합니다 |
| ★**홀드아웃 1차** (고치기 전 · 10건) | 6 / 10 (60.0%) | 골든셋보다 14.7%p 낮다 — **결함을 드러냈다** |
| ★**홀드아웃 2차** (고친 뒤 · 14건) | 11 / 14 (78.6%) | 고친 것이 처음 보는 문항에서도 통했다 |
| ★**홀드아웃 3차** (또 고친 뒤 · 14건) | **9 / 14 (64.3%)** | ★★**떨어졌다** — 골든셋과의 격차 14.7 → **31.7%p** |
...
4  ★agent.py 가 라우팅에 «오염된 지침»을 쓰고 있었다
   라우터만 v2 로 바꾸고 agent.py 는 GUIDE_V1 을 직접 불렀다
   고치니 평균 73.3 → 74.7% (폭 안) 인데 «폭»이 16.0 → 4.0%p
   ⇒ 오염 제거는 점수가 아니라 «일관성»을 만들었다

5  ★홀드아웃 60.0% — 골든셋(74.7%)보다 14.7%p 낮다
   평가셋을 «보며» 지침을 고쳤으니 그만큼 과적합돼 있었다
```
