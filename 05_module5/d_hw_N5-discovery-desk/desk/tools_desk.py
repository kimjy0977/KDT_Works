...
# ★의문사는 «버리지» 않고 주제어로 «옮긴다».
#
#   1차에는 의문사를 통째로 불용어에 넣었다. 그랬더니
#     「공룡 언제 멸종」 → [공룡, 멸종]
#     「공룡 왜 멸종」   → [공룡, 멸종]     ← «똑같아진다»
#   카드가 「공룡 멸종 시점」·「공룡 멸종 원인」 둘이라 «동점»이 되고 되묻는다.
#
#   ★그런데 도구 선택 지침(PICK_GUIDE)은 「무엇을 묻는지(언제·왜)까지 넣어라」고
#     말하고 있었다. 모델이 시키는 대로 넣어도 «도구가 그걸 버렸다» —
#     지침만 고치고 도구는 안 고친 자리다(§H-4 와 같은 종류).
#
#   ⇒ 의문사가 «무엇을 묻는지»를 말해 주므로, 버리지 말고 카드의 말로 옮긴다.
