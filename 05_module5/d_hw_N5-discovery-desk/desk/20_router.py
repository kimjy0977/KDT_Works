...
# ★V3 — 혼동행렬이 「PALEO -> CONCEPT 한 방향」을 가리켰다.
#   f1 만 봤으면 「PALEO 가 좀 약하다」로 끝났을 것을,
#   «어느 쪽으로» 새는지 보이자 고칠 자리가 하나로 좁혀졌다.
#   바꾼 것은 «의문사 함정 경고 4줄»뿐 — 나머지는 한 글자도 안 건드렸다.
GUIDE_V3 = """너는 과학 질문을 분류하는 라우터다. 아래 다섯 중 «하나»를 고른다.
...
겹칠 때 — ★«대상이 있나»로 가른다. 이것이 유일한 기준이다:
⚠ 「어떻게」·「언제」·「왜」가 붙었다고 CONCEPT 가 «아니다».
  ★«묻는 방식»이 아니라 «대상이 있나»로 가른다.
  「삼엽충 눈은 어떻게 생겼나요」 → 삼엽충이 있다 → PALEO
  「성운은 언제 생겼나요」       → 성운이 있다   → SPACE
- 대상이 있으면 «그 대상»의 라우트다
  「혜성 꼬리가 왜 둘로 갈라지나요」 → 혜성이 있다 → SPACE
  「성벽이 언제 쌓였는지 어떻게 정하나요」 → 성벽이 있다 → ARCHAEO
- 대상이 없고 «방법·말»만 물으면 CONCEPT 다
...
GUIDES = {"v1": GUIDE_V1, "v2": GUIDE_V2, "v3": GUIDE_V3}
...
def main():
    ...
    ap.add_argument("--guide", choices=["v1", "v2", "v3"], default="v3",
