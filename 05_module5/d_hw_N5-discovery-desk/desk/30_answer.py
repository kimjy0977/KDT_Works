# -*- coding: utf-8 -*-
"""⚠ 구버전 — 지금 쓰는 파이프라인은 `agent.py` 입니다.

지우지 않는 이유
  측정_답변_7b_v1~v4.txt 네 개가 «이 파일의 결과»다.
  지우면 REPORT 개선 이력의 그 숫자들이 «출처 없는 값»이 된다.
  무엇으로 잰 숫자인지 남기려고 둔다 — 실행 경로에서는 빠져 있다.

무엇이 달라졌나
  이 파일: 도구 선택과 답변 작성을 «두 번 호출»로 나눈 초기 구현
  agent.py: LangGraph 한 흐름 + 넘기기 게이트 2개 + 멀티턴
"""
