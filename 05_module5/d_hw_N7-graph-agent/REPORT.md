...
**결과: 사전 1 → 242개.** 배정 150 · 쪼갠 이름 29 · **미분류 28** ·
★모델이 없는 축을 지어낸 건 **0건**.
...
```
적용 «전»  충돌쌍 109개 중 «2편 이상» ★1개
적용 «후»  축·극쌍  6개 중 «2편 이상» ★5개
```
...
**정규화 없이는 영화 사이 다리가 단 1개**였습니다. 이 프로젝트에서 정규화는
장식이 아니라 **작동 조건**입니다.

### 그래프

```
노드 339 (Film 56 · Question 113 · Value 170) · 엣지 392
가로지르는 충돌 53건 · 같은 극이라 버린 것 23건 · 사전에 없어 둔 것 27건 · self-loop 0건
```
...
| 구분 | 문항 | 답변 정확 | 경로 재현 | 평균 초 | basic RAG(BM25) |
|---|---|---|---|---|---|
| 1홉 | 3 | **100.0%** | 83.3% | 1.6 | 0.0% |
| 2홉 | 4 | 50.0% | 58.3% | 1.3 | 0.0% |
| 4홉 | 4 | **75.0%** | 70.0% | 1.2 | 0.0% |
| ★거절 | 3 | **100.0%** | — | 0.5 | ★0.0% (구조적) |
| 전체 | 14 | **78.6%** | | | 0.0% |
...
**[2홉-03] 7번방의 선물 — 탐색 층**
```
경로 재현 0% — 그래프엔 있는데 그 특정 물음-가치 쌍을 안 가져왔다
★원인  영화마다 물음이 2~3개이고 각각 다른 가치를 부른다.
       「가치 대립은?」 한 문항에 «정답이 여럿»인데 골든셋은 하나를 지정했다
★이건 시스템 결함이 아니라 ★문항 설계의 한계다 — 정직하게 남긴다
```

**색인 0건** — 기대 삼중항이 그래프에 «없는» 경우는 없었습니다. 추출·정규화는 새지 않았습니다.
...
**③ ★분산 측정** — 이번엔 1회만 재고 냈습니다. `--repeat` 옵션은 만들어 뒀지만
돌리지 않았습니다. 앞 과제에서 **같은 골든셋을 3회 돌려 폭 6.7%p**를 본 적이 있어,
**한 번 잰 78.6%를 「성능」이라 부르기엔 이릅니다.**
