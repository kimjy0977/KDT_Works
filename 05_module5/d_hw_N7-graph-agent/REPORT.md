...
> ← 실행 방법·폴더 구조·피어리뷰 안내는 **[README.md](README.md)**
...
⚠ ★**「홉 수」는 «문항의 난이도»이지 «실행 설정»이 아닙니다** (피어리뷰 지적).
넷 다 **같은 `max_hop=4`** 로 돕니다. 표가 「1홉 실행 / 4홉 실행」을 견준 것처럼
읽힐 수 있어 적어 둡니다 — 갈라 놓은 것은 **기대 경로의 길이**입니다.
홉 수를 «설정»으로 바꿔 가며 견주는 것은 아직 안 했습니다(§6 참조).
...
#### ★거절이 «어느 층»에서 일어나는지 — 셋이 다릅니다 (피어리뷰 지적)

제출본은 거절을 **구조적**이라고만 적었습니다. 실제로 재 보니 **3건 중 2건은 아닙니다.**

```
거절-01   근거 ★0개        → 탐색 층. 가져올 것이 없어 «구조적으로» 거절
거절-02   근거 150개 받고   → ★생성 층. 모델이 「이 중엔 제작비가 없다」고 판단
거절-03   근거 117개 받고   → ★생성 층. 〃 (감독 관계를 안 뽑았으므로)
```

⇒ ★**둘은 «모델이 거절해 주는» 것입니다.** 3/3 으로 안정적이지만,
구조가 막아 주는 것과 **모델이 안 지어내 주는 것은 보증의 세기가 다릅니다.**
거절-01 은 코드가 보장하고, 거절-02·03 은 **프롬프트가 부탁하는** 것입니다.
`output/runs.jsonl` 의 `evidence_all` 길이로 층을 가를 수 있습니다.
...
```
★정규화 전  두 편 이상이 함께 부르는 가치 «다리»  2개  (가치 쌍 150)
★정규화 후  〃                                  6개  (축·극 쌍 8)

노드 487 · 엣지 661 · 삼중항 495 · 문서 78
축을 가로지르는 충돌 107건 · 같은 극이라 버린 쌍 48건
```

★**「미분류」는 세 가지를 가리킵니다** — 세는 대상이 다릅니다.

| 이름 | 값 | 무엇을 세나 | 출처 |
|---|---|---|---|
| 충돌 쌍 미분류 | **10** | 축 8개에 안 담긴 «대립 쌍» | `graph_stats.unmapped` |
| 사전 미분류 | **6** | 사전에 남긴 «가치 이름» | `values.json._미분류` |
| 배정 회차 미분류 | §2 참조 | «마지막 배정 한 회차» 기록 | `values.json._배정기록` |

*이 블록은 `output/graph_stats.json` · `values.json` 에서 `sync_numbers.py` 가 찍어냅니다 — 손으로 옮기지 않습니다.*
...
|---|---|---|---|---|---|
| 1홉 | 3×3 | **100.0%** | 100.0% | 1.4 | 0.0% |
| 2홉 | 4×3 | **100.0%** | 100.0% | 1.5 | 0.0% |
| 4홉 | 4×3 | **100.0%** | 100.0% | 1.5 | 0.0% |
...
### ★★100% 가 된 경위 — «성능»과 «채점»을 나눠 적습니다

| 시점 | 전체 | 무엇이 바뀌었나 | 어느 쪽인가 |
|---|---|---|---|
| 코퍼스 56건 · 1회 | — | 4홉 75.0% | — |
| 코퍼스 78건 · 3회 | 85.7% | ★좋아하는 7편을 포함해 코퍼스를 늘렸다 | **성능** |
| 골든셋 수정 · 3회 | **100.0%** | ★**채점 잣대를 내가 고쳤다** | **채점** |

**그때 무엇이 0/3 이었나** (코퍼스 78건 · 골든셋 수정 «전»)

```
★흔들림 0개 / 14개
★구조적 실패 2개 — «항상» 실패
  2홉-02  서브스턴스    답 「내면화된 폭력」 / 기대 ['자기 인식','자아 정체성']
  2홉-04  이미테이션 게임 답 「개인의 프라이버시」 / 기대 ['과학적 진보','인간적 윤리']
안정      나머지 12개 모두 3/3
```

★**두 답이 다 맞았습니다.** 서브스턴스는 「여성의 신체와 노화에 대한 사회적 압박」이라는
물음도 던지고, 그 물음이 **「내면화된 폭력」을 부릅니다.** 그래프에 그렇게 들어 있습니다.
**골든셋이 «한 물음»의 가치만 정답으로 뒀던 것**이 문제였습니다.

```python
# make_goldenset.py — 고치기 전
"reference": vals,        # 그 «물음»이 불러낸 가치만

# 고친 뒤 — 그 «영화»가 부르는 가치 전부
all_vals = [v2 for q2 in ASKS(film) for v2 in INVOKES_VALUE(q2)]
"reference": all_vals or vals,
```

⇒ ★**이건 점수를 올리려고 잣대를 늘린 것과 구별돼야 합니다.** 구별 근거 셋:

1. **`reference_contexts` 는 그대로 뒀습니다** — 경로 재현율은 여전히
   «그 특정 경로»를 타는지 잽니다. 그래서 경로 재현율은 **45~58% 로 낮게 남아 있습니다.**
   느슨하게 고쳤다면 이 값도 같이 올랐을 겁니다.
2. **답변 정확 기준은 안 건드렸습니다** — 「기대 정답의 조각이 답에 있으면 1」 그대로입니다.
3. **채점기를 오답으로 검증했습니다** — 아래 「채점기가 관대해진 건 아닌가」.

⇒ 그래도 **14문항은 제가 냈고 홀드아웃이 없습니다.** 100% 는 «내가 낸 문제를 내가 푼 값»
입니다. 이 숫자보다 **basic RAG 대조(전 구간 0.0%)** 와 **경로 재현율(45~58%)** 이
더 정직한 지표라고 생각합니다.

### ★★경로 재현율 45~58% 는 «채점 결함»이었습니다 — 피어리뷰에서 나왔습니다

제출본에는 이렇게 적혀 있었습니다.

```
답변 정확   100.0%      기대 정답의 «조각»이 답에 있나
경로 재현   45.8~58.3%  기대 삼중항 (주어,목적어) 쌍이 실제 근거에 있나
```

그리고 그 간격을 **「같은 가치에 «다른 물음»으로 닿아도 답은 맞다」**로 설명했습니다.
그럴듯했고, **틀렸습니다.**

오재호님(이슈 #1)이 코드에서 찾아 주셨습니다 —
...
**실측 — gather 45건 중 ★39건(87%)이 60 상한에 걸렸고, 실제 개수 중앙값은 192였습니다.**
⇒ 기대 삼중항이 **61번째 뒤에 있으면 «그래프에 있어도» 실패로 찍혔습니다.**
...
**★지표가 헐거워진 것이 아닌지 대조했습니다.** 근거가 192개면 엣지 661개의 29% 입니다 —
「뭘 넣어도 통과」일 수 있습니다. 그래서 **그래프에서 무작위로 192개**를 뽑아
같은 채점기에 넣어 봤습니다(20회).
...
⇒ ★**100% 는 진짜입니다.** 그리고 **제 진단이 틀렸다는 것이 더 중요한 결과**입니다.
「다른 길로 닿는다」는 설명은 **측정값에 맞춰 지어낸 이야기**였고,
실제로는 **채점기가 자기 손을 묶고 있었습니다.**

★**배운 것** — 하네스에 「측정값이 예상과 어긋나면 «설명»하지 말고 «확인»한다」고
적어 두고도, 어긋난 값에 **설명을 붙여 REPORT 에 실었습니다.**
남이 코드를 읽어 주지 않았으면 못 찾았을 자리입니다.

<span class="fine">답변 프롬프트는 여전히 `evidence[:60]` → 앞 40개만 씁니다.
**토큰 비용은 안 늘었습니다.** 바뀐 것은 «채점»뿐입니다.</span>
...
> ⚠**이 절은 「골든셋 수정 «전»」의 기록입니다.** 지금은 실패 0건입니다.
> **지우지 않고 남깁니다** — 실패가 어떻게 0이 됐는지가 여기 들어 있고,
> 0건만 보이면 **「원래 잘 됐다」로 읽힙니다.** 아래 셋 중 둘은
> «문항 설계의 한계»였고, 그걸 읽어내서 골든셋을 고쳤습니다.
> (그때의 `2홉-04` 는 「80일간의 세계 일주」입니다 — 시드 7편을 우선하도록
> 문항 뽑는 규칙을 바꾸기 전이라, 지금의 `2홉-04`(이미테이션 게임)와 다릅니다.)
...
### ★★이름이 거짓말을 하고 있었습니다 — 「회피」가 아니라 「표시」입니다

`is_hub` 를 계산은 하는데 **경로를 거르지 않습니다.** `agent.py` 는 허브를
`hub_used` 에 **기록만 하고 그대로 붙입니다.**

```python
if is_hub:
    st.setdefault("hub_used", []).append(v)   # ★기록만 한다. 거르지 않는다
```

**그리고 거르지 «않는» 것이 맞았습니다.** 데모의 인터스텔라 4홉 답은
**「가족애」(허브)를 통과**합니다. 걸렀으면 **그 답이 통째로 사라집니다.**
⇒ 코드를 고치는 대신 **이름을 고쳤습니다** — 하는 일이 「표시」이니 그렇게 부릅니다.

★**그리고 「거의 발동하지 않는다」도 틀렸습니다.** 세어 보니 —

```
임계 9%      넘는 Value 3개
             생명 존중 9편 11.7% · 정의 7편 9.1% · 가족애 7편 9.1%
발동         ★42건 중 18건 (43%)   가족애 9 · 생명 존중 6 · 정의 6
```

56건 코퍼스의 「최대 11%」를 78건에서도 그대로 적어 뒀던 것입니다.
**수치를 옮겨 적었더니 코퍼스가 자라는 동안 그 자리에 남았습니다** —
§3 의 성능표가 낡았던 것과 **같은 종류의 사고**입니다.
...
어떤 대립을 공유하는지 말해 주세요」 — 한 화면에 **답변 · 탄 경로 117개 ·
근거 삼중항 60개 · 출처**가 함께 나옵니다.*

### 신경 쓴 것 넷

**① ★「탄 경로」를 답변만큼 크게 보입니다.**
답만 보이면 그래프를 썼는지 알 수 없습니다. 4홉 경로를 펼쳐 놓습니다 —

`인터스텔라 ─ASKS─ 과학과 인간적인 감정은 어떻게 조화를 이룰 수 있는가?`
`  ─INVOKES_VALUE─ 가족애`
`  ─INVOKES_VALUE← 가난한 자들이 부유한 자의 삶에 기생하는 것은 정당한가?`
`  ─ASKS← 기생충`

위 화면의 경로 10개가 **전부 「가족애」를 한가운데 두고** 다른 영화로 건너갑니다
(그 아래 「… 외 107개」). **두 영화가 «어디서» 이어졌는지**가 답과 같은 화면에 있습니다 —
이 시스템이 하는 일이 그것뿐이고, 화면이 그걸 숨기지 않습니다.

**② ★거절을 «오류»가 아니라 «기능»으로 보입니다.**
근거가 없으면 「근거를 찾지 못했습니다」를 그대로 띄웁니다.
평가셋 드롭다운 라벨에 **「1홉/2홉/4홉/★거절」**을 적어, 채점자가
**거절 문항을 바로 골라 눌러 볼 수** 있게 했습니다 — 거절은 숨길 것이 아니라
**보여 줘야 하는 동작**입니다.

**③ 왼쪽에 그래프의 «크기»와 «임계값»을 함께 둡니다.**
Film 77 · Question 155 · Value 255 · 엣지 661 — 답이 **어느 크기의 그래프에서
나온 것인지**를 먼저 보입니다. 홉 수 슬라이더 아래에는 허브 임계를 적고,
**「★거르지는 «않습니다»」**와 **지금 허브인 가치 셋**(생명 존중 12% · 정의 9% ·
가족애 9%)을 함께 띄웁니다. ★이 목록은 **그래프에서 «세어»** 보여 줍니다 —
전에 「최대 11%」를 손으로 박아 뒀더니 코퍼스가 자라는 동안 옛 값이 남았습니다.
설정을 보이면서 **그 설정이 실제로 무엇을 하는지**를 같이 적었습니다.

**④ ★URL 로도 질문할 수 있습니다.**
`http://localhost:8511/?q=질문&hop=4` 로 열면 질문·홉이 채워지고 바로 실행됩니다.
질문이 **평가셋 문항이면 드롭다운도 같이 맞춥니다** — 위 캡처의 드롭다운이
`[4홉] 영화 《인터스텔라》…` 로 잡혀 있는 것이 그것입니다.
**링크 하나로 «이 결과 화면»을 그대로 건넬 수** 있고, 아래 캡처 스크립트도
이 경로를 씁니다.
...
```python
pick = st.selectbox(...)
q = st.text_input("질문", value=pick)   # ★두 번째 실행부터 안 먹는다
```

streamlit 은 위젯이 한 번 렌더된 뒤의 `value=` 변경을 **무시합니다.**
그래서 **드롭다운에서 문항을 골라도 질문 칸이 안 채워졌습니다.**
요건 ⑥ 「사람이 직접 써 볼 수 있는 화면」이 **반쯤만 작동**하고 있었는데,
저는 늘 직접 타이핑해서 **혼자 쓸 때는 한 번도 안 걸렸습니다.**
`key` + `session_state` + `on_change` 로 고쳤습니다.

**★`hop` 파라미터를 «안 읽고» 있었습니다 — 문서가 코드보다 앞서 있었습니다.**
README 와 REPORT 에 `?q=질문&hop=4` 라고 적어 뒀는데, 코드는 `q` 만 봤습니다.
**캡처가 4홉이었던 건 기본값이 4라서**였습니다 — 그래서 아무도(저도) 못 알아챘습니다.
파라미터 읽기를 **사이드바보다 «위»로 올려** 슬라이더에 닿게 고쳤습니다
(위젯이 렌더된 «뒤»에는 못 바꾼다 — 드롭다운이 죽어 있던 것과 **같은 원인**입니다).

⇒ ★**「적어 뒀으니 된다」가 아닙니다.** 문서에 쓴 기능은 **한 번 눌러 봐야** 합니다.

**문항이 이 프로젝트를 가리키지 않았습니다.**
드롭다운에 **오펜하이머·인터스텔라·컨택트가 하나도 없었습니다.**
1홉 영화를 하드코딩했고, 4홉은 축마다 «첫 영화»를 뽑아 같은 영화가 두 번 들어갔습니다.
**출발점인 7편을 우선으로** 고르게 바꿨습니다 (`make_goldenset.py` 의 `SEED`).

⇒ ★**둘 다 「내가 다니는 길」로만 다녀서 안 보였습니다.**
  데모 캡처를 찍으려고 **처음으로 «남이 다니는 길»**(드롭다운)을 밟아 보고 나왔습니다.
  §3 의 측정은 전부 `evaluate.py` 로 돌린 것이라, **화면을 안 거칩니다** —
  **측정이 100% 여도 화면은 고장 나 있을 수 있습니다.**

### 캡처를 «어떻게» 찍었나 — `capture_demo.py`

공개 저장소에 올리는 이미지라 **개인 정보가 찍히면 안 됩니다.**
손으로 찍은 첫 두 장에 **탭 그룹과 북마크바**가 들어가 지웠습니다.
그래서 **찍는 일 자체를 스크립트로** 만들었습니다.

```bash
streamlit run app.py --server.port 8512     # 띄워 두고
python capture_demo.py                      # docs/demo.png 로 저장
```

```
헤드리스 크롬 + 빈 프로필   ★북마크·탭·확장·줌이 «없는» 곳에서 연다.
                           브라우저 UI 가 아예 없으니 «안 찍힌다».
CDP 로 직접 말한다          websockets 만 쓴다 (playwright 안 깔았다)
★「근거 삼중항」이 뜰 때까지 «본다»
                           고정 sleep 을 쓰지 않는다 — LLM 호출 시간이
                           매번 다르고, 짧게 잡으면 «빈 화면»을 찍는다
```

★**스크립트로 만든 진짜 이유는 «다시 찍기» 입니다.**
사이드바 문구를 고쳤는데 **캡처에는 옛 문구가 그대로 남아 있었습니다.**
손으로 찍으면 다시 안 찍습니다 — 한 줄이면 다시 찍습니다.
⇒ **화면 · 캡처 · 캡처 설명은 «셋이 한 묶음»**입니다. 하나를 고치면 셋을 고칩니다.
...
정규화 후    ★6개
```
<span class="fine">★실측값은 §2 의 표(`graph_stats.json` 에서 찍어냅니다)를 보십시오.
제출본의 이 자리에는 **56건 코퍼스 시절 값(1→5)이 남아 있었습니다** — 피어리뷰 지적.</span>
...
그래서 **어느 축에도 안 맞으면 미분류로 남기고 목록을 냈습니다.**

⚠ ★**「미분류」라는 한 단어가 «세 가지 다른 것»을 세고 있었습니다** (피어리뷰 지적).
제출본은 여기서 28, §2 에서 8 이라고 적었는데 **둘 다 같은 말을 쓰고 다른 것을 셌습니다.**
숫자를 맞추는 것으로는 부족해서 **이름을 갈랐습니다** — §2 의 표를 보십시오.
...
**① ~~경로 재현율 45.8~58.3%~~ → ★원인은 «채점 결함»이었습니다.**
제출 뒤 피어리뷰(오재호님)에서 나왔습니다. 답변용으로 60개로 자른 목록을
**채점에도 그대로** 썼고, 87%가 상한에 걸려 있었습니다. 풀고 다시 재니 **전 구간 100%**
(무작위 대조 29.2% — 지표는 여전히 가릅니다). §3 에 전말을 적었습니다.

★**남은 숙제는 다른 것이 됐습니다** — 이제 경로 재현율이 **너무 쉽습니다.**
근거 192개는 엣지 661개의 29% 이고, 「그 안에 있나」는 **「의도한 추론을 했나」와 다릅니다.**
⇒ 다음에 재야 할 것은 **답변이 «자기가 탄 경로»를 말하게 하고 그것을 대조**하는 쪽입니다.
자른 목록으로 재는 것도, 안 자른 목록으로 재는 것도 **둘 다 그 질문에는 답하지 못합니다.**

**② 문항을 «물음» 단위로 쪼개기** — 2홉이 0/3 이었던 원인이 여기였습니다.
영화 하나가 물음을 2~3개 던지고 각각 다른 가치를 부르는데,
「이 영화의 가치 대립은?」 한 문항에 **정답이 여럿**입니다.
지금은 기대값을 「그 영화가 부르는 가치 전부」로 넓혀 **채점을 맞췄을 뿐**이고,
**문항 자체는 여전히 뭉뚱그려져 있습니다.**

**③ 파편화 — Value 255개 / Film 77편.** 축·극으로 «잇기»는 되지만
**노드 자체는 아직 합쳐지지 않았습니다.** 임베딩으로 유사 가치를 병합하면
다리가 더 늘 것입니다. 다만 그때도 **너무 뭉치면 충돌이 사라지는** 경계를 재야 합니다.
★허브는 **9% 임계로 «표시»만 하고 거르지 않습니다**(42건 중 18건에서 표시).
거르면 데모의 인터스텔라 4홉 답(가족애 경유)이 사라지기 때문입니다.
**언제 걸러야 하는지**를 아직 모릅니다 — 「허브라서 나쁘다」가 아니라
「허브를 타는 답이 **싱거워질 때**」가 기준이어야 할 것 같은데, 그걸 못 쟀습니다.
이 코퍼스의 문제는 허브가 아니라 **파편화**였습니다. 가설이 뒤집힌 자리입니다.

**④ 회차를 더 늘리기** — 3회로는 흔들림의 «폭»을 못 잽니다.
지금 흔들리는 문항이 0개인데, **3회로 「사라졌다」고 단정할 수 없습니다.**
56건 코퍼스에서는 `4홉-02` 가 1/3 이었고, 그때도 3회였습니다.
10회를 돌리면 폭이 나오는데 **비용이 회차에 비례**합니다(3회 = 42호출 ≈ 110원).

**⑤ 「축을 가로지르는 충돌」 107건** — 이 프로젝트에서 제일 흥미로운데 아직 안 썼습니다.
```
...
전부 `output/graph_stats.json` 의 `cross_axis` 에 있습니다.

**⑥ 홀드아웃 평가셋** — 14문항을 제가 냈습니다. 100% 는 «내가 낸 문제를 내가 푼 값»입니다.
제가 안 보고 만든 문항이 필요한데, 그러려면 **사람이 더 있어야** 합니다.
그래서 피어리뷰에서 **문항을 받아 보는 것**이 지금 할 수 있는 가장 가까운 대안입니다.
