...
**모듈5 노드7 [실습 프로젝트]** · 김주영 · 2026-09-22
...
**같은 가치에 «다른 물음»으로 닿아도 답은 맞습니다.** 인터스텔라 → 가족애 로 가는 길이
여럿이고, 제가 기대한 길이 아니어도 답은 같습니다(실측: 4홉 인터스텔라에서 **경로 117개**).

⇒ ★**두 값의 간격이 이 시스템의 정직한 모습입니다.** 답을 맞히는 능력과
«의도한 추론을 하는» 능력이 다르고, 전자만 재면 후자를 못 봅니다.
그래서 두 값을 **같은 표에** 뒀습니다 — 하나만 자랑하면 나머지가 안 보입니다.
...
**① ★경로 재현율 45.8~58.3%** — 여기가 제일 약합니다. 답변은 100% 인데
**「기대한 그 경로」는 절반쯤만 탑니다.** 같은 가치에 «다른 물음»으로 닿아도 답은 맞고,
그래서 **답이 맞았다고 «의도한 추론»을 했다는 뜻이 아닙니다.**
고치려면 기대 경로를 **하나가 아니라 «허용 집합»**으로 두거나,
답변이 **«어느 경로를 썼는지 말하게»** 해서 그걸 대조해야 합니다.
⇒ 답변 정확도를 올리는 것보다 **이쪽이 남은 진짜 숙제**입니다.
