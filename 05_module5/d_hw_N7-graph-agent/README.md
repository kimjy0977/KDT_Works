...
| ★정규화 전/후 | `output/graph_stats.json` | 다리 **2개 → 6개** · 가로지르는 충돌 **107건** · 축 8개 (아래 블록이 이 파일에서 찍힙니다) |
...
```
① ★경로 재현율이 «너무 쉽습니다»   ← 제출본의 0.46~0.58 은 «채점 결함»이었습니다.
   (지금 1·2·4홉 전부 1.00)      답변용으로 60개로 자른 근거를 채점에도 써서
                                 87%가 상한에 걸려 있었습니다(오재호님 지적).
                                 풀고 다시 재니 전 구간 100%, 무작위 대조 29.2%.
                                 ⇒ 그런데 이제 «그 안에 있나»가 너무 헐겁습니다.
                                 「의도한 추론을 했나」는 여전히 못 재고 있습니다.
