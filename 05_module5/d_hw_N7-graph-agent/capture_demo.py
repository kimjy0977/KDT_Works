...
QUESTION = ("영화 《인터스텔라》와 «같은 가치 대립»을 다루는 다른 영화를 "
            "추천하고, 어떤 대립을 공유하는지 말해 주세요.")
READY_MARK = "근거 삼중항"        # 이 글자가 뜨면 답이 다 그려진 것이다
...
def main():
    ...
    ap.add_argument("--port", type=int, default=8512)
    ...
    url = ("http://localhost:%d/?hop=%d&q=%s"
           % (a.port, a.hop, urllib.parse.quote(QUESTION)))
