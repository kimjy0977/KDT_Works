...
def 절제표():
    ...
    # ★같은 설정인 묶음들 — 장치를 끈 것이 아니라 «잡음»을 재는 조건이다
    같은설정 = ("전부 켬 (기준선)", "★기준선 (재측정)", "★축 A — 주제 (우리 설정)")
    pool = [x["근거율"] for r in rows if r["조건"] in 같은설정
            for x in r.get("_회차별", [])]
    ...
    if len(pool) >= 6:
        ...
        out.append("★**평균끼리 견주면 잡음이 «사라진 것처럼» 보입니다** — "
                   "같은 설정 세 묶음의 평균은 %s 로 0.6%%p 안이었습니다. "
                   "원시값을 펴야 보입니다."
                   % " / ".join("%.1f" % (r["근거율"] * 100)
                                for r in rows if r["조건"] in 같은설정))
        out.append("")
