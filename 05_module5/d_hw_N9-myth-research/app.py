...
def _잡음문장():
    """★수치를 «손으로» 적지 않는다 — 측정 파일에서 찍어낸다."""
    r = compare.잡음대차이()
    if not r:
        return ("아직 절제 실험 기록이 없습니다 — "
                "<code>python ablation.py</code> 를 돌리면 "
                "여기에 잡음 폭이 적힙니다.")
    크다 = r["잡음"] > r["차이"]
    return ("미리 적어 둡니다 — 같은 설정을 <b class='num'>%d</b>번 돌려 잰 "
            "잡음이 <b class='num'>%.1f%%p</b>이고, 장치를 바꾼 조건 "
            "<b class='num'>%d</b>개의 차이는 <b class='num'>%.1f%%p</b>입니다. "
            "<b>%s</b> 한 번 돌린 숫자로 판단하지 마세요."
            % (r["n"], r["잡음"] * 100, r["조건수"], r["차이"] * 100,
               "잡음이 차이보다 큽니다."
               if 크다 else "차이가 잡음보다 큽니다."))
...
H('<div class="how">'
  ...
  '<div class="go">↓ 아래 <b style="color:%s">2</b> 축과 규모</div>'
  '</div></div>'
...
# ★한 띠다 — 머리의 「2 축과 규모를 정한다」와 «글자까지» 맞춘다.
#   전에는 여기와 아래가 각각 «2» 를 달아, 번호가 순서의 증거 구실을 못 했다.
H('<div class="bar"><div class="no">2</div>'
  '<div class="lbl">축과 규모</div>'
  '<div class="hint">절을 <b>무엇을 기준으로</b> 나눌지, 몇 명이 몇 건씩 읽을지, '
  '네 장치를 켤지 끌지 — 이 셋이 한 벌입니다.</div></div>')
H('<div class="pair"><b>절을 나누는 축</b>'
  '<span>목차의 축이 곧 분업이고, 분업이 곧 결과물의 모양입니다. '
  '코디에게 <b>이 축으로 나눠라</b>를 직접 말합니다.</span></div>')
...
H('<div class="pair"><b>규모와 장치</b>'
  '<span>몇 명이 몇 건씩 읽을지, 그리고 네 장치를 켤지 끌지. '
  '<b>끄고 다시 돌리면</b> 그 장치가 실제로 값을 하는지 보입니다 — '
  '다만 잡음이 커서 한 번으로는 판단할 수 없습니다.</span></div>')
...
with st.expander("계기판 전체 — 지표마다 어느 장치를 보는 신호인가"):
    # ★한 열에 «문자열과 정수»가 섞이면 Arrow 가 통일을 못 해 표가 통째로
    #   죽는다(ArrowTypeError). 게다가 이건 expander 안이라 ★접혀 있으면
    #   안 보인다 — 캡처를 자동으로 찍다가 드러났다.
    #   ⇒ 화면에 내보내는 열은 «전부 문자열»로 맞춘다.
    st.table([{"지표": k,
