...
# ★--q4 : 축을 «뺀» 질문으로. 주질문은 이미 축을 말해서 축 실험이 안 된다
_QS = json.load(io.open(os.path.join(HERE, "data/questions.json"),
                       encoding="utf-8"))
Q = (next(x["text"] for x in _QS["보조질문"] if x["id"] == "Q4")
     if "--q4" in sys.argv else _QS["주질문"]["text"])
...
CASES = [
    ...
    # ★카드를 «앞부분만» 보던 옛 방식. 배정이 빗나가는지 본다.
    ("카드 앞부분만", {"고른카드": False},
     "도입부만 보고 배정한다 → 읽고안쓴·편중이 뛸 것"),
...
AXIS = [
    # ⚠1차 판 — 역할 «명단»만 바꿨다. 목차가 한 글자만 달랐다(실패한 실험).
    #   ⛔지우지 않는다 — 「실험이 축을 안 바꿨다」도 결과다.
    ("축 A(명단만) — 주제", {}, "역할 명단만 주제로"),
    ("축 B(명단만) — 문화권", {"_축B": True}, "역할 명단만 문화권으로"),
    # ★2차 판 — 프롬프트에 «이 축으로 나눠라»를 직접 넣는다
    ("★축 A(지시) — 주제", {"_축": "주제"},
     "코디에게 «주제로 나눠라»를 직접 말한다"),
    ("★축 B(지시) — 문화권", {"_축": "문화권"},
     "코디에게 «문화권으로 나눠라»를 직접 말한다"),
]
WIDE = [
...
def main():
    ...
    for name, patch, why in cases:
        ...
        _ax = patch.pop("_축", None)
        if _ax:
            # ★역할 «명단»만 바꾸면 축이 안 바뀐다(실측). 프롬프트도 바꾼다.
            agent.ROSTER = (agent.ROSTER_ALT if _ax == "문화권" else _ROSTER0)
            agent.설정["축지시"] = agent._CFG["_축지시"][_ax]
        if patch.pop("_축B", False):        # 옛 이름 — 앞 기록과 호환
