...
# ── 설정 — ★도메인 값은 config.json «하나»가 출처다 ──────────────
#   노드8 실습에서는 이 값들이 여기 박혀 있었다. 요건이 config.json 을
#   따로 요구한 이유는 ★ablation 이 「끈 척」만 하는 사고를 막기 위해서다
#   (흔한 사고 ② — 스위치를 껐다고 로그만 찍고 코드 경로는 그대로).
_CFG = json.load(io.open(os.path.join(HERE, "config.json"), encoding="utf-8"))
설정 = {k: v for k, v in _CFG.items() if not k.startswith("_")}

# ★도메인 이름 — 프롬프트에 들어간다. 주제를 바꾸면 여기만 바뀐다.
도메인 = _CFG["_코퍼스"].get("이름", "세계 신화")

# ★명단 — 역할은 «프롬프트 한 줄»이지 별도의 코드가 아니다.
#   같은 문서를 읽어도 시간순 담당은 날짜를, 인물 담당은 사람을 뽑는다.
ROSTER = _CFG["_역할명단"]["★주제 축(A) — 기본"]
ROSTER_ALT = _CFG["_역할명단"]["문화권 축(B) — 대조 실험용"]
...
def _발췌(t, n):
    """★문서에서 n자를 «고르게» 뽑는다 — 앞·중간·끝.

    왜 — 「앞 250자」만 보면 도입부가 대표성이 없을 때 배정이 빗나간다.
      실측 사고: 「수메르 창조신화」를 창조 절에 배정했는데
      그 문서의 내용 대부분이 ★홍수였다. 창조 절이 홍수 이야기를 썼다.
    ⛔카드를 «길게» 하지 않는다 — 코디가 보는 글자가 늘면 ★격리가 무너진다.
      같은 글자 수로 «어디를» 보느냐만 바꾼다.
    """
    body = DOCS[t]
    if not 설정.get("고른카드", True) or len(body) <= n:
        return body[:n].replace("\n", " ")
    k = n // 3
    a, b = len(body) // 3, len(body) * 2 // 3
    return " … ".join(x.replace("\n", " ").strip()
                      for x in (body[:k], body[a:a + k], body[b:b + k]))


def cards():
    """4강 ② — 제목 + %d자. 코퍼스 전체의 약 1.7%%.

    ★기본은 «앞·중간·끝»에서 고르게 뽑는다(고른카드=True).
      설정["고른카드"]=False 면 옛 방식(앞부분만) — ablation 대조용.
    """
    n = 설정["카드"]
    return "\n".join("- %s: %s" % (t, _발췌(t, n)) for t in sorted(DOCS))
...
def 축지시():
    """★목차를 «어느 축»으로 나눌지 프롬프트에 직접 넣는다.

    ⚠실측으로 알게 된 것 — 역할 명단만 바꿔서는 축이 «안 바뀐다».
      목차를 정하는 것은 역할이 아니라 질문이다.
      축 A·B 두 보고서의 목차가 한 글자만 달랐다(REPORT §6).
    ⇒ 코디에게 «이 축으로 나눠라»를 말해야 실제로 바뀐다.

    설정["축"] 이 없으면 빈 문자열 — 앞 측정과 호환을 깨지 않는다.
    """
    a = 설정.get("축지시")
    if not a:
        return ""
    return ("★이 보고서는 반드시 «%s» 를 기준으로 절을 나누세요.\n"
            "  예: %s\n"
            "  ⛔다른 기준으로 나누지 마세요.\n" % (a["이름"], a["예"]))
...
def plan(st: S) -> S:
    ...
    p = ("다음은 %s 문서 %d건의 «제목 + 앞부분»입니다.\n\n%s\n\n"
         ...
         % (도메인, len(DOCS), cards(), st["question"], n,
            축지시(), "·".join(roster)))
...
def researcher(st) -> S:
    ...
    p = ("너는 «%s»다.\n질문: %s\n맡은 절: %s\n지시: %s\n\n"
         ...
         "  ★★제목은 위 목록에서 «글자 그대로 복사»하세요. 한 글자도 바꾸지 마세요.\n"
         "  ★★꺾쇠를 반드시 « 로 열고 » 로 닫으세요.\n"
         "     ⛔★절의 «마지막» 인용에서 » 대신 $ 를 쓰는 사고가 실제로 난다 (4절 중 3절).\n"
         "     실측 사고 — 「길가메시」를 「길가미시」로 적어 허위 인용 2건으로 찍혔습니다.\n"
         "     내용은 맞았는데 이름이 틀려서 채점기가 «안 읽은 문서»로 봤습니다.\n"
         "메모에 없는 것은 쓰지 마세요.\n\n"
    ...
    # ★형식 깨짐만 고친다. 고친 건수는 «따로» 센다 — 조용히 고치지 않는다.
    원고, _보정 = _꺾쇠보정(원고, read)
...
def _기각말(기각):
    """★버린 두 번째 원고를 말로 남긴다 — 조용히 버리면 아무도 모른다."""
    if not 기각:
        return ""
    return " · ★2차 원고 기각 %d건(%s)" % (
        len(기각), ", ".join("%s 인용 %d→%d" % x for x in 기각))


def _꺾쇠보정(원고, 읽음):
    """★« 로 열고 «다른 글자»로 닫은 인용을 고친다 — 형식만.

    실측 — 절 마지막 인용에서 » 대신 $ 를 쓴다. 프롬프트로 ★안 고쳐졌다.
      그대로 두면 인용 정규식이 못 잡아 «출처가 있는 문장»이
      근거율에서 「없음」으로 세어진다 — 지표가 실제보다 낮게 나온다.

    ⛔내용은 안 건드린다. ★읽은 문서에 «있는 제목»일 때만 닫는다.
      없는 제목이면 그건 형식 문제가 아니라 환각이므로 손대지 않는다.
    반환 — (고친 원고, 고친 건수)
    """
    if not 읽음:
        return 원고, 0
    n = 0
    for t in sorted(읽음, key=len, reverse=True):
        # ★제목 뒤 «닫는 자리»에 » 가 아닌 «한 글자 기호»가 온 것만 바꾼다.
        #   실측된 것은 $ 지만 다른 기호가 올 수도 있으므로 «기호 한 글자»로 본다.
        #   ⛔한글·영문이 이어지면 건드리지 않는다 — 제목의 일부일 수 있다.
        pat = re.compile("«" + re.escape(t) + r"([^»가-힣A-Za-z0-9\s])")
        원고, k = pat.subn("«" + t + "»", 원고)
        n += k
    return 원고, n
...
def review(st: S) -> S:
    # ★두 번째 원고를 어떻게 할 것인가 — 요건이 «이름 붙여» 경고한 자리다.
    #   「무조건 덮어쓰면 인용을 빠뜨린 원고가 멀쩡한 원고를 지웁니다」
    #
    #   전에는 «마지막 바퀴»를 무조건 썼다. 재위임은 「부족하다」고 신고한
    #   절만 다시 내보내는데, 다시 쓴 원고가 «더 나쁠» 수 있다.
    #   실제로 일어난다 — 2바퀴는 시작문서 배정 없이 스스로 고르므로
    #   엉뚱한 문서를 잡으면 인용이 되레 줄어든다.
    #
    #   ★정한 규칙 — «인용이 줄어들면 덮어쓰지 않는다».
    #   왜 인용 수인가: 재위임의 «목적»이 근거를 더 대라는 것이기 때문이다.
    #   목적을 달성 못 한 원고로 갈아 끼울 이유가 없다.
    #   ⛔길이로 재지 않는다 — 길어지는 것은 목적이 아니다.
    #   버린 원고도 «로그에 남긴다» — 조용히 버리면 왜 그대로인지 모른다.
    latest, 기각 = {}, []
    ...
    if not 설정["재위임"] or wheel >= 설정["최대바퀴"] or not 빈칸:
        ...
        return {"wheel": wheel, "_배차": [], "종료": why,
                "log": ["④ 점검  %d절 중 빈 칸 %d개 · 종료(%s)%s"
                        % (len(latest), len(빈칸), why, _기각말(기각))]}
    ...
    return {"wheel": wheel, "toc": toc, "_배차": 배차, "종료": "",
            "log": ["④ 점검  %d절 중 빈 칸 %d개 → %d명 재위임%s"
                    % (len(latest), len(빈칸), len(배차), _기각말(기각))]}
