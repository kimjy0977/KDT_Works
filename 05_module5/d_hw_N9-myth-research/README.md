...
조건                     근거율        편중      중복률   안쓴  호출   자수
 전부 켬 (기준선)              65.8%±7.3   26.8%    36.8%  0.5   26   2510
 배정 끔                    70.6%±25.2  34.9%    49.3%  1.2   30   2609
 구역 끔                    67.2%±8.2   32.7%    31.2%  0.8   26   2443
 역할 끔                    76.2%±21.2  25.7%    30.6%  2.0   26   2560
 재위임 끔                   67.3%±10.9  29.7%    36.8%  0.8   26   2444
★기준선 (재측정)               65.2%±15.0  24.6%    29.9%  0.8   26   2535
★축 A — 주제 (우리 설정)        65.2%±11.1  25.2%    36.8%  0.5   26   2521
★축 B — 문화권               64.5%±14.4  23.7%    41.7%  1.2   26   2500
★solo (대조군)              20.6%±39.1  14.7%     0.0%  4.2   14   1692
```
...
```
같은 설정 12회 «원시값»
  57.5 · 60.0 · 60.0 · 62.2 · 63.9 · 65.1 · 65.8 · 66.7 · 69.4 · 70.7 · 71.0 · 72.5
  최소 57.5%  최대 72.5%   →  ★잡음 폭 15.0%p

장치를 바꾼 조건들의 «평균» 범위   64.5 ~ 76.2%  →  차이 11.7%p

⇒ ★잡음(15.0)이 차이(11.7)보다 «크다».
  이 지표로는 ★어떤 장치도 가를 수 없다.

단 하나 예외 — solo
  solo  0.0 · 17.4 · 25.8 · 39.1
  solo 최대 39.1%  vs  팀 최소 57.5%   →  ★겹치지 않는다
```

★**평균끼리 견주면 잡음이 «사라진 것처럼» 보입니다** — 같은 설정 세 묶음의 평균은 65.8 / 65.2 / 65.2 로 0.6%p 안이었습니다. 원시값을 펴야 보입니다.

*이 블록은 `output/ablation.json` 에서 `sync_numbers.py` 가 찍어냅니다 — 손으로 옮기지 않습니다. 4회 평균 ± 폭.*
...
```bash
streamlit run app.py        # 로컬 실행이 필수 요건입니다
```

### 이 화면이 하는 일

```
표제       문서 42건 → 넷이 나눠 읽음 → 보고서 한 편
           ★장식 이미지가 아니라 «코퍼스 그 자체»입니다.
             막대 하나가 실제 문서이고 길이가 실제 글자 수입니다
조작       질문 · ★절을 나누는 축(직접 입력 가능) · 규모 · 장치 넷
           칸마다 «무엇을 하는 칸인지» 한 줄이 붙어 있습니다
★배정 도면  왼쪽 코퍼스 → 선 → 오른쪽 조사관
           겹친 선 = 중복(×) · 몰린 선 = 편중 · 흐린 선 = 읽고 안 씀
           조사관은 «색»이 아니라 ★선 모양으로 갈립니다
판독       도면에서 읽히는 것을 «문장»으로. ⛔지표 배너 줄을 쓰지 않습니다
★잡음 띠    같은 설정을 12번 돌렸을 때의 범위를 그리고 이번 값에 바늘을 꽂습니다
           바늘이 띠 «안»이면 ★이 값으로는 아무것도 못 읽습니다
★회차 비교  두 번 돌리면 회차가 «쌓이고» 둘을 골라 견줄 수 있습니다
...
결정의 출처는 **[DESIGN.md](DESIGN.md)** 한 곳이고, `ui.py` 는 그것을 코드로
옮긴 것뿐입니다.

```
이 시스템이 하는 일 = 자료를 구역으로 나눠 맡기고 추적하는 것 → 그건 «측량»이다
  도면은 흰 종이에 그린다        → 미색 배경. ⛔다크모드 안 씀
  도면에 그림자는 없다            → box-shadow 0 · 그라디언트 0
  도면은 «선»으로 구분한다        → 카드 대신 괘선
  범례는 색이 아니라 «선 모양»    → 조사관을 dash 로 가른다 (색맹도 갈린다)
  붉은색은 «검토 표시»다          → 유채색 ★단 하나. 근거·경보에만
```

⛔**2판을 버렸습니다.** 다크 + 명조 + 금박 + 올캡스였는데, 조사해 보니
2026년 AI 생성 UI의 표식 16개 중 **9개**를 밟고 있었습니다(카드 왼쪽 컬러 보더 ·
그라디언트 · 스탯 배너 줄 · 1·2·3 번호 단계 …). 그리고 **「안티-슬롭」 조언 자체가
새 클리셰**였습니다 — 크림 배경 + Instrument Serif + 세이지 그린.
`redteam.py` 가 이 표식들을 **검사로 박아** 두고 있습니다.

본문 대비 **17.0:1** · 강조 6.5:1 · 경보 글자 5.6:1 ·
⛔이모지를 구조 아이콘으로 쓰지 않음 · `prefers-reduced-motion` 존중.
