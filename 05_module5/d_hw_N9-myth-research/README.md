...
python redteam.py             # ★제출물 자체를 친다 (50항목)
```
...
**「성좌 필사본」** — 장식이 아니라 «내용»에서 끌어냈습니다.

```
밤하늘        신화가 새겨진 곳          → 심야 남색 (순흑 금지)
4개의 성좌    조사관 넷                 → 각자 «구역». ★구역은 실제 기능이다
금박 brass    인용·근거                 → 근거가 붙으면 금이 박힌다
지상/지하     ★컨텍스트 격리            → 원문은 아래에 잠기고 원고만 올라온다
붉은 인장     경보                      → 순색 빨강이 아니라 «봉인»
```

첫 탭이 계기판이 **아니라 절별 추적**입니다 — 요건이 「숫자만 보여 주는 화면은 이 프로젝트의 요점을 놓친다」고 못 박았습니다.

⛔이모지를 구조 아이콘으로 쓰지 않았습니다(전부 인라인 SVG) · 본문 대비 **14.4:1** · 포커스 링 · `prefers-reduced-motion` 존중.
토큰은 `ui.py` **한 곳**에만 있습니다 — 같은 색을 두 군데서 정하지 않으려고요.
