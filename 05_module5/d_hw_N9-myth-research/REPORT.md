...
![데모 화면 — 표제부터 배정 도면·판독까지](docs/demo-map.png)

★위 한 장이 **전 과정**입니다 — 표제 · 사용법 세 걸음 · 1 질문 · 2 축과 규모
· 실행 · **3 배정 도면** · 판독 · 잡음 띠. 조작부만 크게 본 것은
[docs/demo.png](docs/demo.png) 입니다.

⛔**손으로 찍지 않습니다.** `capture_demo.py` 가 빈 프로필 헤드리스 크롬을
띄워 찍습니다 — ①개인 정보(탭·북마크)가 안 찍히고 ②화면이 바뀌면
**한 줄로 다시 찍을 수 있어** 실제로 다시 찍게 됩니다.
...
### ★「뭘 눌러야 하는지 모르겠다」 — 색이 아니라 «면»이 없었다

주영님 지적 — 「텍스트 색깔이 다 똑같다 보니 뭘 눌러야 되는지 모르겠다」.
**맞는 지적이었고, 원인은 색의 «가짓수»가 아니었습니다.**

```
NN/g       그림자를 뺐으면 ★색이 두 배로 일한다.
           글자 4.5:1 만이 아니라 ★조작요소 «경계»가 3:1 (WCAG 1.4.11)
Atlassian  평면 디자인의 elevation 은 그림자가 아니라 ★면 색의 차이
GitLab     면이 안 갈리면 조작요소가 «배경의 일부»로 읽힌다
```

⇒ **미색을 «페이지»에서 «조작면»으로 옮겼습니다.**

```
전   페이지가 미색 · 조작요소는 그 위 연한 테두리   ⇒ 글과 안 갈림
후   페이지는 밝게 · 조작요소만 미색 면 + 진한 경계  ⇒ 누를 곳이 떠 보인다
```

★**새 색을 들이지 않았습니다.** 쓰던 미색의 **자리만** 옮겼습니다.
제도판 위에 종이를 얹은 모양이라 「측량 도면」 은유도 그대로이고,
**그림자는 여전히 0** 입니다.

★그리고 이 결정은 **잴 수 있게** 만들었습니다 — 경계색을 고를 때
**3:1 을 넘는 가장 밝은 값**을 찾아서 골랐고(`#9A9281` · 3.01:1),
`redteam.py` 가 그 값을 **매번 다시 잽니다.** 처음 고른 `#9C9483` 은
**2.93:1 로 0.07 모자라 `assert` 에 걸렸습니다** — 눈으로는 몰랐을 차이입니다.

### ★그물에 구멍이 셋 있었습니다 — 이 화면을 고치다 드러났습니다

이 프로젝트가 내내 말한 것이 **「그물은 재 봐야 구멍을 안다」**인데,
정작 **이 프로젝트의 그물**에 같은 구멍이 있었습니다.

```
① 「문법 OK」인데 앱은 죽어 있었다
   ui.css() 는 % 포매팅을 쓴다. CSS 에 width:100% 를 그냥 적었더니
   ★ast.parse 는 통과하고 «부르는 순간» TypeError 로 죽었다.
   화면은 빨간 에러 한 장인데 레드팀은 「문법 ui.py OK」라고 했다.
   ⇒ ★파싱은 실행이 아니다. 이제 redteam 이 ui.css() 를 «실제로 부른다».

② 전역 클래스 이름이 겹쳐 «먼 곳»이 조용히 망가졌다
   띠 안 소제목에 .sub 를 새로 지었는데, 괘선()·표제가 이미 쓰던 이름이었다.
   .sub{display:flex} 가 안내문에도 걸려 ★<b> 하나하나가 flex 항목이 되어
   문장이 줄줄이 끊겼다. 고친 곳이 아니라 ★엉뚱한 곳이 깨졌다.
   ⇒ redteam 이 겹치는 최상위 클래스를 «센다».

③ 손으로 박은 수치가 또 거짓말이 됐다 — ★네 번째 같은 사고
   빈 화면 안내에 「12번 돌려 15.0%p · 차이 11.7%p」가 박혀 있었다.
   회차가 15번으로 늘자 그대로 거짓말이 됐다(실측 25.6%p · 17.8%p).
   ⇒ compare.잡음대차이() 한 곳에서 찍어낸다. sync_numbers 도 같은 것을 쓴다.
```

★**③ 은 이 프로젝트에서 네 번째입니다** — ①config.json 밖의 도메인 값
②metrics 밖의 판정 ③「같은 설정」 목록이 두 파일에 ④이번.
**규칙은 하나인데 네 번 샜습니다.** 적어 둔다고 안 새는 게 아니라,
**찍어내게 만들어야** 안 샙니다.

### 단계 번호를 «실제 띠»에 — 안내가 안내 구실을 못 했다

머리에 1·2·3 을 써 놓고 **정작 아래 띠에는 번호가 없었습니다.**
「3 도면을 읽는다」인데 **도면이 어디 있는지 화면이 안 알려 줬습니다.**
⇒ 띠마다 번호를 붙이고, 사용법 칸에 **「↓ 아래 2 축과 규모」** 처럼
**어디로 가는지**를 적었습니다. 두 곳의 **글자를 똑같이** 맞췄습니다 —
다르면 같은 것을 두 이름으로 부르게 됩니다.

⚠처음엔 축과 규모를 **두 띠로 나눠 놓고 둘 다 «2»** 를 달았습니다.
**같은 번호가 둘이면 번호가 순서의 증거 구실을 못 합니다.** 한 띠로 합쳤습니다.

### ★캡처가 «죽어 있던 표»를 찾아냈습니다 — 두 번째로 UI 가 버그를 드러냈다

도면까지 담으려고 캡처를 다시 찍었더니 **90초를 기다려도 안 떴습니다.**
서버 로그를 보니 —

```
pyarrow.lib.ArrowTypeError:
  ("Expected bytes, got a 'int' object",
   'Conversion failed for column 값 with type object')
```

계기판 표의 `값` 열에 **문자열(「3건」·「67.6%」)과 정수가 섞여** 있었고,
Arrow 가 타입을 통일하지 못해 **표가 통째로 죽으면서 그 뒤 렌더링이 멈췄습니다.**

★**이건 `expander` 안이라 «접혀 있으면 안 보입니다».**
사람이 손으로 찍었다면 **끝까지 몰랐을 버그**입니다.
자동 캡처가 «모든 것을 그려 보기» 때문에 드러났습니다.

```
①  숫자만 볼 땐 안 보였다 → 도면을 그리니 판정 이중화가 드러났다
★② 손으로 찍을 땐 안 보였다 → 자동으로 찍으니 죽은 표가 드러났다
```

⇒ **화면에 내보내는 열은 전부 문자열로 맞춥니다.**
그리고 규칙이 하나 더 생겼습니다 — ★**「그려 봐야 안다」가 두 번 맞았습니다.**
파싱도, 눈대중도 실행이 아닙니다.
...
```
단계                   명령                           걸린 시간  산출물
OK   ① 코퍼스 수집           python fetch_corpus.py       57.0초  data/corpus.json
OK   ② 질문 하나 실행         python run.py                20.2초  output/run.json
OK   ③ 대조군 공정성          python baseline.py --fairness    1.8초  -
OK   ④ 대조군 실행           python baseline.py           38.4초  output/baseline.jsonl
OK   ⑤ 비교 보고서 3편        python make_compare.py       76.5초  output/reports
OK   ⑥ 표 찍어내기           python sync_numbers.py        0.3초  -
OK   ⑦ 레드팀              python redteam.py             0.2초  -

총 194.4초 · ★7 / 7 단계 통과  (코퍼스는 캐시 · --fresh 로 전부 새로)
```
...
검사는 — 재현(파일·README 명령) · 비밀(키·`.env`) · 문서가 «지금»을 말하나 ·
