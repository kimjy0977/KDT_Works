...
```
① ★축 실험이 «지표로는» 결론을 못 냈다
   차이가 잡음 안에 있다. 사람이 읽어 판단한 것(§6)이 유일한 근거다

② 회차가 적다
   노드8 에서 3회 결론이 ★4회차에 깨졌다(solo 근거율 21.7% → 16.3%).
   여기서도 4회는 «분포를 안다»고 할 만한 수가 아니다
...
### ★신경 쓴 것 — 「숫자만 보여 주는 화면은 요점을 놓친다」

요건의 이 문장이 이 화면의 설계 전부입니다. 그래서 **첫 탭이 계기판이 아니라
「절별 추적」**입니다.

```
탭1 ★절별 추적    절을 펼치면 그 조사관이
                  ① 누가(역할) ② 배정받은 일 ③ 무엇을 읽었나
                  ④ 무엇을 썼나 ⑤ 어디를 인용했나
                  + ★조사관이 «본» 원문 메모 (접어 둠)
탭2 보고서        완성본 + 내려받기
탭3 파이프라인     로그 · 종료 사유 · 비용 · ★코디가 «질문이 온 뒤» 정한 목차
```

### 읽고 안 쓴 문서를 ⬜ 로 표시합니다

```
✅ «대홍수 신화»  34,037자     ← 읽고 인용까지
⬜ «길가메시»     14,641자     ← ★읽었는데 안 썼다
```

숫자 「읽고 안 쓴 1건」으로는 **어느 문서인지 모릅니다.**
목록에 표시하면 **배정이 왜 엉뚱했는지**가 바로 보입니다.

### ★격리를 «보이게» 하는 것이 두 번째 일

원문이 조사관 안에서 끝난다는 말은 **글로는 와닿지 않습니다.**
그래서 「조사관이 본 원문 메모」를 **접어서** 두고, 펼치면
「**이 메모를 코디네이터는 본 적이 없습니다**」라고 적었습니다.
그리고 아래에 **코디가 본 글자 / 팀이 읽은 글자**를 원시값으로 나란히 둡니다.

### 경보와 신호를 «갈라» 보여 줍니다

경보(허위 인용 · 표기 흔들림 · 인용 0곳 절 · 꺾쇠 안 닫힘)가 하나라도 0 이 아니면
**빨간 박스**로 올라옵니다. 신호는 metric 카드와 접이식 표에 둡니다.
⇒ **섞어 놓으면 무엇이 급한지 모릅니다.**

### 왼쪽에서 «끄고» 견줄 수 있습니다

장치 넷을 체크박스로, 축을 라디오로 뒀습니다.
**같은 질문을 끄고 켜며 돌려 보면** §5 의 표가 «왜 그렇게 나왔는지»가 손에 잡힙니다.
...
```
평균끼리 견주면   65.8 / 65.2 / 65.2   → 0.6%p. ★안정적으로 «보인다»
...
```
── WARN (1) ──
  루브릭 ③ 엔드투엔드 오류 없이                  REPORT 에 1/3 키워드

합계 50항목 · ★BAD 0 · WARN 1
```
