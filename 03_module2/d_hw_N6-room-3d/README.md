...
폰으로 15~26초 걸어 다니며 찍은 영상 한 개에서 3차원 공간을 복원해 웹에 올린 것입니다.
...
```
폰 영상(1920x1080, 26초) → 사진 24장 → 3D 점 336만 개 → 가우시안 알갱이 → .sog 14MB → 이 주소
```
...
| 단계 | 내용 | 결과 |
|---|---|---|
| 복원 | `facebook/map-anything-apache` (Apache-2.0), 코랩 T4, fp16 | 점 3,362,068개, 카메라 24 |
| 검증 | 카메라 이동폭/장면 대각선 = 3.66/6.35 = **0.577** (>0.1 정상) | 걸어 찍음 확인 |
| 알갱이 | 점→가우시안 스플랫 `.ply` (17칸, 68바이트/알갱이, f_rest 없음), y·z 부호 뒤집기 | 228MB |
| 압축 | `@playcanvas/splat-transform` → `.sog` | 14.3MB (14.2배) |
