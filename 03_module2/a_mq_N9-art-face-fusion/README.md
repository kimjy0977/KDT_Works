...
## 9. `legacy-drafts/` — 정본이 아닌 것들

만드는 «과정»에서 남은 것을 모아 두었다. **제출물도 최신본도 아니다.**

- `원본소재/` — 입력 이미지 4장 (여기 `results/` 는 결과만 있고 입력이 없었다)
- `문제정의서_명화합성AI_초안.md` — 위 `문제정의서.md` 의 초안
- ★`문제정의서_패키지목업AI.md` — **채택하지 않은 아이디어.** 시각디자인 도메인이라
  나중에 다시 꺼낼 만해서 남겼다
- `제출_체크리스트.md` — 제출 당시 점검표

자세한 것은 [`legacy-drafts/README.md`](legacy-drafts/README.md).
