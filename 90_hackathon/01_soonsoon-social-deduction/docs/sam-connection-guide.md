...
| 궤도 정거장 | 60 | 40 | 4 | `--outside=0.6 --kitchen=9,30` 이 필요했다 (아래) |
| ~~고딕 저택~~ | 14 | 15 | 4 | ❌ **폐기** — 어두운 데다 침실을 9개밖에 안 그렸다 |

#### ⛔ 그림은 1024×1024 가 한계다 — 세 품질 전부 확인

「맵을 더 키우자」가 나올 때마다 확인하게 되므로 결론을 박아 둔다.

| 품질 | 나온 크기 | 비고 |
|---|---|---|
| `low` | **1024×1024** | 기본 |
| `medium` | **1024×1024** | 131 SSAM · 크기 그대로 |
| `high` | **1024×1024** | 크기 그대로 |

**격자(`C`/`R`)를 키우는 건 그림을 키우는 게 아니다.** 자르는 칸 수만 늘어난다:

```
1024 ÷ 32 = 32px   ← 게임 타일과 1:1. 손실 없음
1024 ÷ 45 = 22.7px ← 45x45 로 하면. 게임에서 32px 로 그리니 1.4배 늘림 + 칸 경계 어긋남
1024 ÷ 64 = 16px   ← 64x64. 2배 늘림
```

**`themeTileSize` 를 64 로 바꾸면 `1024x1024 / 32x32 -> 64` 로 표시된다.**
이건 「32px 타일을 64px 로 **늘려 그리겠다**」는 뜻이지 원본이 커지는 게 아니다.

> ⚠ **우주선 맵이 뿌연 이유가 바로 이것이었다.** `tileset_ship.png` 는 2048×2048 인데
> 원본 `source_ship.png` 는 1024다 — **2배로 늘려 만든 것**이다.

**진짜로 키우려면 1024짜리 여러 장을 이어 붙이는 수밖에 없다**(2×2 = 2048 → 64×64 타일).
다만 ① 넉 장의 화풍·이음새를 맞춰야 하고 ② 64×64 는 **이동만 하다 단계가 끝난다**는
이유로 우주선이 이미 반려됐다.

**대신 쓴 방법: 같은 32×32 안에서 「걸을 수 있는 칸」을 늘린다.**
프롬프트에 「벽은 머리카락처럼 얇게 · 복도는 아주 넓게 · 회색이 화면의 절반」을 넣으면
보행 427 → **557칸**(1인당 43 → 56칸)이 된다. **화질 손실 0 · 이동 시간 그대로.**

#### 정거장에서 새로 붙인 옵션 둘

**`--outside=0.6`** — 건물 밖 우주가 넓게 잡히는 그림. 「테두리에서 닿는 바닥은 바깥」
판정에 기본 3할 한도가 걸려 있는데, 정거장은 바깥이 절반을 넘어 안 걸렸다.
실제로 **어두운 구석 두 곳에 스폰이 잡혔다.**

**`--kitchen=x,y`** — 부엌을 손으로 찍는다. 부엌은 「복도와 같은 바닥색」으로 찾는데
(프롬프트가 「방마다 다른 색·복도는 한 가지」를 시키므로), **정거장 그림은 갤리 바닥이
파랑이고 선실 하나가 회색**이라 거꾸로 잡혔다. **그림이 지시를 어기면 자동 규칙은
방법이 없다** — 그땐 눈으로 보고 찍는 게 맞다.

같이 고친 것: **복도 조각 걸러내기**. 허브 고리 벽이 복도를 잘라 **복도 토막 두 개가
후보에 섞였고**, 「제일 멀리 흩어진 것」 규칙이 하필 그걸 뽑아 **복도 한복판이 누군가의
「자기 방」**이 됐다. 복도색인 덩어리는 부엌 하나만 빼고 전부 침실 후보에서 뺀다.

#### ⭐ 방을 넉넉히 주문하고 골라 쓴다

고딕 저택은 「열 개 침실」을 네 번 주문해 **네 번 다 아홉 개**가 나왔다.
같은 자리에 **열두 개를 주문**했더니 열두 개가 나왔고, 파이프라인이 **멀리 흩어진 열 개**를
골라 썼다. 남는 둘은 그냥 빈 방으로 둔다 — 문제가 안 된다.

**모자란 것보다 남는 게 낫다.** 모자라면 스폰이 복도에 잡혀 「자기 방」이 안 되지만,
남으면 아무 일도 안 일어난다.

밝기도 같이 중요하다. 저택은 「촛불 밝기」라 벽과 바닥이 둘 다 어두워 **어느 임계값으로도**
못 갈랐다. 정거장은 「밝은 흰 패널 + 단색 바닥」이라 한 번에 됐다.
→ **어두운 컨셉은 평면도로 안 맞는다.** 분위기는 게임 안 조명(밤 장막)으로 만든다.
