...
> 2026-08-19 해커톤 세션 2차. **소스를 직접 열어서 확인한 것만** 적었다. 추측은 `[추정]` 표기.
...
## 6. 해커톤 세션 3차 추가 조사 (2026-08-19) — 스프라이트·프롭
...
### 6-7. `TileMapSystem` 은 타일셋이 **단수**다 ★★★★★

`new TileMapSystem({ tileSet })` — 하나뿐이다. 레이어마다 다른 시트를 쓰려면
**레이어마다 타일맵 GameObject 를 따로** 만들어야 한다(6-5 와 같은 이유로 어차피 갈라야 한다).

우리 `world.js` 는 시각 레이어 하나당 타일맵 하나를 만들고, sortingLayer 를
배열 순서대로 준다(`back` 은 -110 부터, `front` 는 15 = 캐릭터 10 위 · 이름표 20 아래).
빈 레이어는 아예 만들지 않는다.
