...
1. ~~근접 대화~~ ✅ 완료 (성공기준 §2·§3)
2. ~~AI 4명 이동~~ ✅ 완료
3. ~~게임 루프~~ ✅ 완료 (저녁→밤→아침→토론→투표→판정)
4. **기술 문서** — 성공기준 §4. **1급 산출물이고 아직 없다. 다음은 이것.**
4-b. **밤 시뮬 랜덤 배정** — `game-spec.md` §2-1(범인 매 판 랜덤·플레이어 포함)이 **아직 미구현**.
   지금은 고정 시나리오(범인=살살이) 한 판뿐이라 두 번째 플레이의 재미가 없다.
   서버에 밤 시뮬레이터를 두고 `/api/newgame` 으로 기억을 생성해야 한다(정답은 서버에만).
