...
★이 스크립트의 자리 (TH-20260915-SCOPE ★종결문 C-1 · 78·90행)
  카드 «등재»는 ★매니저가 한다. 미등재가 나오면
  매니저가 카드를 넣고 «만든 쪽에 알린다». 값이 필요하면 만든 쪽에 묻는다.
  감사는 판정이 아니라 «그물»이다 — 누가 옳은지 가리는 게 아니라 빠진 걸 줍는다.

  ⛔2026-09-23 정정 — 여기 «만든 쪽이 등재한다»고 적혀 있었다.
    그건 토론 «종결 전» 매니저 주장이고, 토론은 그 반대로 끝났다
    (후보1 «생산자» 기각 · 36~38행 — 「생산자는 잊는다」).
    종결 후 이 주석을 안 고쳐서, 튜터가 이 줄을 근거로
    매니저 관할 파일을 커밋했다(d73c021). ★도구 안의 낡은 주석은
    «문서»가 아니라 «지시»로 읽힌다 — 규칙이 뒤집히면 도구부터 고친다.
...
def main():
    ...
    for w in missing:
        print("     %s" % w)
    print()
    print("  ⇒ ★매니저가 카드를 넣고 만든 쪽에 알린다(TH-20260915-SCOPE C-1).")
