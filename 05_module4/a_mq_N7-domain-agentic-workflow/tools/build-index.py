...
PAT = {
    ...
    'size': r'크기는\s*([^.]{1,60})',
    # ⚠«[^.]» 로 잡으면 괄호 «안»의 마침표에서 잘린다 —
    #   「루브르 박물관 (수장번호 AO 6620. 파리)」 같은 경우 「파리)」만 남는다.
    #   실제로 정답지에 「1), 뉴욕, 미국」·「BURGM-paoil174)」 같은 쓰레기가 들어가 있었고,
    #   그 상태로 채점하면 «모델이 맞혀도 틀린 것»이 된다. 문장 경계(". ")를 기준으로 되짚는다.
    'collection': r'(?:^|\.\s)([^.]*(?:\([^)]*\)[^.]*)*?)에 소장되어 있다',
    'era': r'사조는\s*([^.]{1,50})',
