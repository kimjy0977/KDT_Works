...
# Upstream container roots that benchmark_source.relocate() maps into the
# trial workspace. Kept beside the mapping it serves.
...
class LocalBenchmarkTools:
    ...
    def relative_path(self, value: str) -> str:
        ...
        # Local-port fix: the system prompt promises "/app -> app,
        # /protected -> protected, /db -> db, /home/user -> home/user".
        # On Windows a POSIX-rooted path has no drive, so is_absolute() is
        # False and the mapping below never runs. Strip the documented root
        # slash lexically so the promised contract holds on every platform.
        # This grants no new access: ".." is already rejected above and
        # LocalTools.path() still applies every containment check.
        text = value.replace("\\", "/")
        if text.startswith("/"):
            trimmed = text.lstrip("/")
            head = trimmed.split("/", 1)[0]
            if trimmed and head in PORTED_ROOTS:
                path = Path(trimmed)
        if path.is_absolute():
